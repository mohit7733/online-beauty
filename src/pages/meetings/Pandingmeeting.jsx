...
function Pandingmeeting() {
            ...
            .then(result => {
                if (result?.success == false) {
                    ...
                    setTimeout(() => {
                        ...
                        result.data?.media_files.map((item, i) => {
                            if (item?.media_type === "image") {
                                ...
                                setlink("");
                                setselect(
                                    Number(result.data?.company?.thumb_index)
                                );
                            }
                        });
                    }, 100);
                    if (
                        ...
                        setbuyersector(
                            JSON.parse(result.data?.company?.sector)
                        );
                    }
                }
            })
    ...
    const handleRefuseMeeting = () => {
        axios
            .get(
                `${api}/api/v1/supplier-meeting-refused?meeting_id=${state.id}`,
                {
                    headers: {
                        Authorization:
                            "Bearer " + localStorage.getItem("token"),
        ...
        <>
                                        ...
                                        <div className="row justify-content-between">
                                            <div className="col_left last-contnt">
                                                {productData?.questions
                                                    ?.sort((a, b) =>
                                                        a.company_question_id >
                                                        b.company_question_id
                                                            ? 1
                                                            : -1
                                                    )
                                                    ?.map(item => {
            ...
            <Modal
                ...
                <div className=" calendar_fix calendar-wrapper">
                    ...
                    {showTP ? (
                        <Timepicker setTime={setSTime} sTime={sTime} />
                    ) : null}
                </div>
